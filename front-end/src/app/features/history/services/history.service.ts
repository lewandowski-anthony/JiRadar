...
export class HistoryService {
  ...
  private readonly historySignal = signal<any | null>(null);

  readonly history = this.historySignal.asReadonly();

  getUserHistories(tracker: string, projects: string[], startDate: string, endDate: string, page: number, size: number): Observable<any> {
    ...
    return this.httpClient.get<any>(url, {
      ...
      map(response => {
        const parsedBody = autoMapSnakeToCamel<any[]>(response.body || []);
