...
export function JiraLoginForm({ onSuccess, onError, loginFn, loading }: FormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            onError(err.response?.data?.message || "Token Jira invalide.");
        }
    };
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1">
                ...
                <input
                    ...
                    disabled={loading}
                />
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Token Atlassian API</label>
                <input
                    type="password"
                    value={token}
            ...
            <button type="submit" disabled={loading} className="w-full mt-2 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 font-semibold text-sm text-white transition-colors disabled:opacity-50">
                {loading ? "Vérification..." : "Se connecter à Jira"}
            </button>
        </form>
    );
}