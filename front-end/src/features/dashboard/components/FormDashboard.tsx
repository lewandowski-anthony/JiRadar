...
export function FormDashboard({onSubmit, isLoading}: DashboardFormProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!filters.projectKey || filters.projectKey.trim().length < 2) {
            setError(t.error.projectCodeRequired);
            return;
        }
        setError(null);
        onSubmit(filters);
        ...
        <form onSubmit={handleSubmit} className="bg-slate-900 p-6 rounded-xl border border-slate-800 space-y-4 mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="flex flex-col">
                    <label className="text-slate-400 text-sm mb-1" htmlFor="projectKey">{t.form.projectCode}</label>
                    <input
                        type="text"
                        id="projectKey"
                        name="projectKey"
                ...
                <div className="flex flex-col">
                    <label className="text-slate-400 text-sm mb-1" htmlFor="startDate">{t.form.startDate}</label>
                    <input
                        type="date"
                        id="startDate"
                        name="startDate"
                ...
                <div className="flex flex-col">
                    <label className="text-slate-400 text-sm mb-1" htmlFor="endDate">{t.form.endDate}</label>
                    <input
                        type="date"
                        id="endDate"
                        name="endDate"
                ...
                <div className="flex flex-col">
                    <label className="text-slate-400 text-sm mb-1" htmlFor="granularity">{t.form.granularity.title}</label>
                    <select
                        id="granularity"
                        name="granularity"
                        ...
                        className="bg-slate-950 border border-slate-800 rounded p-2 text-slate-200 focus:outline-none focus:border-blue-500"
                    >
                        <option value="NONE">{t.form.granularity.noGranularity}</option>
                        <option value="DAY">{t.form.granularity.daily}</option>
                        <option value="WEEK">{t.form.granularity.weekly}</option>
                        <option value="MONTH">{t.form.granularity.monthly}</option>
                        <option value="YEAR">{t.form.granularity.yearly}</option>
                    </select>
                </div>
            </div>
            ...
            <div className="flex justify-end">
                ...
                >
                    {isLoading ? t.app.loading : t.form.updateDashboard}
                </button>
            </div>
        </form>
    );
